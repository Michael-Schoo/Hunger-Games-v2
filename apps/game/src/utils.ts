...
// function to generate age based on num - most age is between 25-40
const amountOfPeople = INITIAL_PEOPLE_PER_DISTRICT
export function chooseAge(num: number) {
    // use a seed based on the num and the amount of people to ensure consistency
    const seed = (num * 320) % amountOfPeople;
    ...
    const u1 = (seed + 1) / (amountOfPeople + 1); // uniform random number between 0 and 1
    const u2 = (seed + 2) / (amountOfPeople + 2); // another uniform random number between 0 and 1
...
export function randomSort<T>(array: T[]): T[] {
    ...
    return array.sort(() => {
        ...
        arrayBufferIndex += 2;
        // console.log({ randomValueA, randomValueB, v: randomValueA > randomValueB })
        // console.log(randomValueA > randomValueB ? 1 : 0)
